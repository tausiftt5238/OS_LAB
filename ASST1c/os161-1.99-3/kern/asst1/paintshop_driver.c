...
void customer(void *unusedpointer, unsigned long customernum)
{
  ...
  do {
    ...
    //for (j = 0; j < PAINT_COMPLEXITY; j++) {
      //can.requested_colours[j] = (random()%(NCOLOURS)) + 1;
    //}
		can.requested_colours[0] = RED;
		can.requested_colours[1] = BLUE;
		can.requested_colours[2] = GREEN;
#ifdef PRINT_ON
		P(print);
    kprintf("C %ld requesting with the following %d, %d, %d\n", customernum,
	    can.requested_colours[0],
	    can.requested_colours[1],
	    can.requested_colours[2]);
		V(print);
#endif

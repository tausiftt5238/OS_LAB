...
void * take_order(void)
{
    ...
    P(QSem);
    if(no_of_customers==0) {
			//int i = 10000;		//yeah because i was getting tired of this. -_-
			//while(i--);
...
void fill_order(void *v)
{
  	...
  	int i,col,lockedcol=0;
  	P(tintBlockerBlock);
  	for(i = 0; i < PAINT_COMPLEXITY; i++) {
   		col = order->can->requested_colours[i] - 1;
      	if(col > 0) {
		//if a customer requests the same tint twice,don't block it
      		if((lockedcol&(1<<col))==0) //n-th bit of lockedcol is 1,if it has already been requested
        		P(tintSem[col]);   //block on the tints required
      		lockedcol = lockedcol|(1<<col); 
		}
	}
	V(tintBlockerBlock);
...
void paintshop_open(void)
{
    ...
    if(bufferBlocker == NULL){
		...
		tintBlockerBlock = sem_create("tintBlockerBlock Sem", 1);      //making sure one tint isn't used by multiple staffs simultaneously
    if(tintBlockerBlock == NULL){
        panic("paintshop_open: out of memory\n");
    }	
...
{
		sem_destroy(bufferBlocker);
    sem_destroy(QSem);
		sem_destroy(tintBlockerBlock);
    int i;
    for(i = 0; i < NCOLOURS; i++)
